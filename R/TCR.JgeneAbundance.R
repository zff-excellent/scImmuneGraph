...
#'
#' BasicPlot(TCR.JgeneAbundance, contig_list, sample_name)
TCR.JgeneAbundance <- function(contigList, sampleName) {
  CreateList <- function(contig_df, sampleNames) {
    # --------------------
    # TRA_TRB_JGene
    TRAB_jgene_Abun <- contig_df %>%
      dplyr::select(barcode, TCRA_jgene, TCRB_jgene) %>%
      unique() %>%
      tidyr::gather(chain, jgene, TCRA_jgene, TCRB_jgene) %>%
