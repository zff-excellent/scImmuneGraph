...
BCR.ContigList <- function(datasetDir, sampleName, groupName) {
  CreateList <- function(datasetDir, sampleNames, groupNames) {
    ...
    filtered_contigs_tb <- read.delim(file = file.path(datasetDir, sampleNames, "filtered_contig_annotations.csv"), header = TRUE, sep = ",", stringsAsFactors = FALSE) %>%
      subset(chain %in% c("IGH", "IGK", "IGL") & toupper(high_confidence) == "TRUE" & toupper(productive) == "TRUE" & toupper(full_length) == "TRUE") %>%
      dplyr::arrange(barcode, factor(chain, levels = c("IGH", "IGK", "IGL")))

    # unique IGH(s)-IGK/IGL(s) pair
    ...
    clonotypes_tb <- filtered_contigs_sub %>%
      ...
      tidyr::separate(clonotype, c("IGH", "IGK.L"), sep = "\\:", remove = F) %>%
      tidyr::separate(IGH, paste0("IGH_", c("vgene", "dgene", "jgene", "cgene", "cdr3aa", "cdr3nt")), sep = "_", remove = T) %>%
      tidyr::separate(IGK.L, paste0("IGK.L_", c("vgene", "dgene", "jgene", "cgene", "cdr3aa", "cdr3nt")), sep = "_", remove = T) %>%
